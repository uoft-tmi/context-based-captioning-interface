...
export default function Home() {
  ...
  const [checking, setChecking] = useState(false); //const [checking, setChecking] = useState(true);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-dynamic px-4">
      <div className="glass-panel w-full max-w-md p-8 sm:p-10 space-y-6 relative overflow-hidden">
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 rounded-full bg-[var(--brand)] opacity-10 blur-2xl"></div>
        <div className="absolute bottom-0 left-0 -ml-8 -mb-8 w-32 h-32 rounded-full bg-[var(--brand-accent)] opacity-10 blur-2xl"></div>

        <div className="text-center relative z-10">
          <h1 className="text-3xl font-bold tracking-tight text-[var(--foreground)] drop-shadow-sm">
            Lecture Captioning
          </h1>
        </div>
        ...
        <div className="relative z-10 mt-8">
          <Auth
            ...
            appearance={{
              ...
              className: {
                container: 'w-full',
                button: 'transition-all duration-300 ease-in-out hover:-translate-y-0.5 shadow-sm hover:shadow-md font-medium',
                input: 'transition-all duration-300 ease-in-out focus:ring-2 focus:ring-[var(--brand)] focus:ring-opacity-50 shadow-sm',
                label: 'text-sm font-semibold text-[var(--text-secondary)] mb-1.5',
                divider: 'my-6',
